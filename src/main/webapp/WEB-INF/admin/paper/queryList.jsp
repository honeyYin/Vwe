	...
	<td align="center">
		...
		<c:otherwise>
		 | 
			<c:choose>
				<c:when test="${item.hasAudit == true}">
					<a id ="${item.id}" href="<%=basePath%>paper/cancleAudit?paperId=${item.id}&channelId=${channelId}&pageNo=${pageNo}&queryTitle=${queryTitle}&type=${type}&isDraft=${isDraft}" class="pn-opt">取消发布</a>
				</c:when>
				<c:otherwise>
